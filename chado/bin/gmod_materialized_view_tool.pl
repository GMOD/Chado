...
sub view_info {
	...
	while(my $row = $sth->fetchrow_hashref()){	
		unless($i){
			...
			$table->[0] = ["MV Name", "Status", "Time to Update"] if $longarg eq "status";
		}
		my @vals = ();
...
sub print_table {
	...
	if(ref($table) eq "ARRAY"){
		foreach my $row (@$table){
			...
			unless((scalar @$row) == $num_cols){
				print "This row has " . scalar(@$row) ." entries, when the number of columns should be $num_cols:\n";
				print join("\t", @$row);
				$valid = 0;
			}
	...
	die "Argument to print_table() must be a rectangular two-dimensional hashref, with values being scalars\n" unless $valid;
...
sub prompt_create_mv {
	...
	my $name = validate({
		prompt => "Give your materialized view a name (word characters only): ",
		test => sub {
				my $t = shift;
				my %exists = ();
				foreach my $n (@NAMES) { $exists{$n} = 1 }
				if($exists{$t}){
					print "MV '$_' already exists!  Current names taken: " . join(", ", @NAMES) . "\n";
				}
				unless($t =~ /^\w+$/){
	...
	my $location = validate({
		prompt => "Where will this MV be located? (schemaname.tablename): ",
		regexp => '^\w+\.\w+$'
		});
	...
	my $mv_specs = validate({
		prompt => "Enter specifications for the MV, OR provide a file in which the specs are written ('? for help): ",
		test => sub {
					my $t = $_[0];
					if(-f $t){
						print "'$t' is a valid file, reading into specifications variable...\n";
	...
	my $query = validate({
		prompt => "Enter the SQL query for the materialized view, or a file containing only the query: ",
		test => sub {
					my $t = @_[0];
					if(-f $t){
						print "'$t' is a valid file, reading into query variable...\n";
						print "File '$t' contents: ";
						$t = slurp_file($t);
						print "$t\n";
					}
					if($t =~ /^\s*SELECT.*FROM[^?]+$/i){
						...
						print "'$t' is not a valid file, or it is not valid SQL.  You can't use placeholders, by the way.\n";
					}
					return 0;
				}
	...
	my $insert_q = $dbh->prepare("INSERT INTO $SCHEMA.$TABLE (last_update, refresh_time, name, mv_schema, mv_table, mv_specs, query) VALUES (NOW(), ?, ?, ?, ?, ?, ?)");
	...
	$dbh->commit();
	print "MV Entered into the registry.\n";
}
...
#	print "Using this query to count total entries: $count_query\n";
	...
	while(my $entry = $sth->fetchrow_hashref()){
		...
		my $query = "INSERT INTO $mv_schema.$mv_table ( " . join(", ", @valid_cols) . " ) ";
