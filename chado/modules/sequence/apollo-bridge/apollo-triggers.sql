...
BEGIN
 ...
 IF (f_name IS NULL and f_type=f_type_gene) THEN
     ...
     IF (f_type_temp=f_type_transcript or 
         ...
         SELECT INTO maxid to_number(max(substring(name from (length(f_name_gene)+1+length(f_type_temp)))), ''99999'') FROM feature where name like f_name_gene||''-''||f_type_temp||''%'';
