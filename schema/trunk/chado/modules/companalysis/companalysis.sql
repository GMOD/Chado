-- $Id: companalysis.sql,v 1.32 2007-02-20 16:44:09 briano Exp $
-- ==========================================
...
COMMENT ON COLUMN ANALYSIS.IDENTITY IS 'Percent identity between the locations compared.  Note that these 4 metrics do not cover the full range of scores possible; it would be undesirable to list every score possible, as this should be kept extensible. instead, for non-standard scores, use the analysisprop table.';
COMMENT ON COLUMN ANALYSIS.SIGNIFICANCE IS 'This is some kind of expectation or probability metric, representing the probability that the analysis would appear randomly given the model. As such, any program or person querying this table can assume the following semantics:
...
COMMENT ON COLUMN ANALYSIS.NORMSCORE IS 'This is the rawscore but
...
COMMNENT ON COLUMN ANALYSIS.RAWSCORE IS 'This is the native score generated by the program; for example, the bitscore generated by blast, sim4 or genscan scores. One should not assume that high is necessarily better than low.';
