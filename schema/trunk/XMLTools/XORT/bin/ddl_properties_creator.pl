...
while (<IN>){
 ...
 if ($input =~/create\s+table/ || $input =~/create\s+TABLE/ || $input =~/CREATE\s+TABLE/){
   $start=1;
   my @temp;
 ...
 if ($start==1){
   if (($input !~/unique|primary|foreign|create|CREATE|TABLE/) || ($input =~/uniquename/ && $input =~/text/) ){
     ...
     if ((($input =~ /ot/ && $input =~/null/) ||($input =~ /NOT/ && $input =~/NULL/) ) && ($temp[1] ne $table_id)){
